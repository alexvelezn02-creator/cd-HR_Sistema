...
def main():
    ...
    if menu == "Calendario Operativo":
        st.title("📅 Calendario de Cumplimiento Legal")
        st.caption("Gestión cronológica de obligaciones para Ecuador")
        ...
        # Listas para alimentar tanto el calendario visual como la tabla de pendientes
        eventos_calendario = []
        eventos_lista = []

        # 1. Eventos legales fijos anuales
        eventos_fijos = [
            {"Fecha": date(year, today.month, 10), "Tarea": "🔴 Pago Aportes IESS", "Link": None, "Tipo": "IESS", "color": "#ff4b4b"},
            {"Fecha": date(year, today.month, 20), "Tarea": "🟡 Cierre de Nómina", "Link": None, "Tipo": "Nómina", "color": "#f1c40f"},
            {"Fecha": date(year, 3, 15), "Tarea": "🟢 Décimo Cuarto", "Link": None, "Tipo": "Décimo", "color": "#2ecc71"},
            {"Fecha": date(year, 12, 24), "Tarea": "🟢 Décimo Tercero", "Link": None, "Tipo": "Décimo", "color": "#2ecc71"},
            {"Fecha": date(year, 4, 15), "Tarea": "🔴 Pago Utilidades", "Link": None, "Tipo": "Utilidades", "color": "#ff4b4b"},
        ]
        ...
        for ev in eventos_fijos:
            ...
            if ev["Fecha"] >= today:
                eventos_lista.append(ev)

        # 2. Dinámicos: SUT Ingresos (+15 días)
        for _, row in ingresos.iterrows():
            f_limite = datetime.strptime(row['fecha_ingreso'], '%Y-%m-%d').date() + timedelta(days=15)
            diff = (f_limite - today).days
            color = "#ff4b4b" if diff < 0 else ("#f1c40f" if diff <= 5 else "#2ecc71")
            icono = "🔴" if diff < 0 else ("🟡" if diff <= 5 else "🟢")
            
            tarea = f"{icono} SUT: {row['nombre_completo']}"
            eventos_calendario.append({"title": tarea, "start": str(f_limite), "color": color})
            
            if f_limite >= today:
                eventos_lista.append({"Fecha": f_limite, "Tarea": tarea, "Link": "Datos Empleados", "Tipo": "SUT"})

        # 3. Dinámicos: Salidas (+7 días para liquidación)
        for _, row in salidas.iterrows():
            ...
            tarea = f"{icono} Liq: {row['nombre_completo']}"
            eventos_calendario.append({"title": tarea, "start": str(f_limite), "color": color})
            
            if f_limite >= today:
                eventos_lista.append({"Fecha": f_limite, "Tarea": tarea, "Link": "Auditoría de Salidas", "Tipo": "Liquidación"})

        # 4. Manuales
        for _, row in manuales.iterrows():
            f_limite = datetime.strptime(row['fecha'], '%Y-%m-%d').date()
            diff = (f_limite - today).days
            color = "#ff4b4b" if diff < 0 else ("#f1c40f" if diff <= 5 else "#3498db") # Azul para manuales futuros
            icono = "🔴" if diff < 0 else ("🟡" if diff <= 5 else "🔵")
            
            tarea = f"{icono} {row['tipo']}: {row['titulo']}"
            eventos_calendario.append({"title": tarea, "start": str(f_limite), "color": color})
            
            if f_limite >= today:
                eventos_lista.append({"Fecha": f_limite, "Tarea": tarea, "Link": None, "Tipo": row['tipo']})

        # --- RENDER DEL CALENDARIO VISUAL ---
        calendar_options = {
        ...
        state = calendar(events=eventos_calendario, options=calendar_options)

        if state.get("eventClick"):
            evento = state["eventClick"]["event"]
            st.info(f"📌 **Detalle del evento:** {evento['title']} — 📅 Fecha: {evento['start']}")

        st.divider()
        
        # --- RENDER DE LISTA INTERACTIVA ---
        st.subheader(f"Próximos a vencer — a partir de {today.strftime('%d/%m/%Y')}")

        if not eventos_lista:
            st.success("No hay tareas pendientes para los próximos días.")
        else:
            eventos_sorted = sorted(eventos_lista, key=lambda x: x['Fecha'])
        ...
        # Gestión de deadlines manuales (para eliminarlos)
        if not manuales.empty:
            with st.expander("⚙️ Administrar deadlines manuales guardados"):
                for _, row in manuales.iterrows():
